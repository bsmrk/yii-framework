...
EOD;
    ...
    private static $iconsUrl = null;
    private static function getIconsUrl()
    {
        if (self::$iconsUrl === null) {
            self::$iconsUrl = Yii::app()->getAssetManager()->publish(
                Yii::getPathOfAlias('system.images.icons')
            );
        }

        return self::$iconsUrl;
    }
    
    public static function icon($name)
    {
        return self::image(self::getIconsUrl() . '/' . $name . '.png');
    }
    
    private static $flagsUrl = null;
    private static function getFlagsUrl()
    {
        if (self::$flagsUrl === null) {
            self::$flagsUrl = Yii::app()->getAssetManager()->publish(
                Yii::getPathOfAlias('system.images.flags')
            );
        }

        return self::$flagsUrl;
    }
    
    public static function flag($countryCode)
    {
        return self::image(self::getFlagsUrl() . '/' . $countryCode . '.png');
    }
}
